...
function Chat({ book, documentId, preselectedCharacterId, onBack }) {
  ...
  const voiceGenerationRef = useRef(null);
  const voiceQueueRef = useRef([]);

  // Check if AI voice is available and preload voice agent
  useEffect(() => {
    const checkAiVoice = async () => {
      try {
        const status = await checkVoiceStatus();
        setAiVoiceAvailable(status.available);
        setVoiceEnabled(status.available);
        console.log("🎤 AI Voice Agent preloaded:", status.available);
      } catch (error) {
        console.error("Failed to check voice status:", error);
        setAiVoiceAvailable(false);
      }
    };
    checkAiVoice();
  ...
  // AI Voice Agent: Generates natural speech using OpenAI TTS (parallel generation)
  const speakWithAIVoiceParallel = async (text, characterId) => {
    if (!aiVoiceAvailable || !isVoiceActive || !text.trim()) return;
    ...
    try {
      ...
      // Cancel any pending voice generation
      if (voiceGenerationRef.current) {
        voiceGenerationRef.current = null;
      }

      // Generate voice (this happens in parallel, not blocking)
      const audioBlob = await generateVoice(documentId, characterId, text, 1.0);
  ...
  const toggleRecording = () => {
    ...
    if (isRecording) {
      ...
      setInputMessage(""); // Clear input before recording
      speechRecognition.start();
      setIsRecording(true);
    }
  };

  // Stop audio when component unmounts or character changes
  useEffect(() => {
    return () => {
      if (currentAudioRef.current) {
        currentAudioRef.current.pause();
  ...
  useEffect(() => {
    if (!selectedCharacter) {
      ...
      if (characterSelectRef.current) {
        characterSelectRef.current.scrollIntoView({
          behavior: "auto",
          block: "start",
        });
      }
  ...
  useEffect(() => {
    ...
    if (preselectedCharacterId && characters.length > 0) {
      const character = characters.find(
        (c) => c.character_id === preselectedCharacterId
      );
      if (character) {
  ...
  const loadDefaultBookCharacters = async () => {
    try {
      ...
      const filteredCharacters = response.characters.filter(
        (char) =>
          !char.name.includes("Company of Dwarves") &&
  ...
  const loadUploadedBookCharacters = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/books/${documentId}/characters`
      );

      if (response.ok) {
        const data = await response.json();
        console.log(
          "✅ Loaded uploaded book characters:",
  ...
  const handleSelectCharacter = async (character) => {
    ...
    // Preload AI voice agent in background (completely non-blocking, fire-and-forget)
    if (aiVoiceAvailable && documentId && character.character_id) {
      // Don't await - let it run in background without blocking UI
      Promise.resolve()
        .then(() => preloadVoice(documentId, character.character_id))
        .then((result) => {
          if (result) {
            console.log(
              `🎤 Voice agent ready for ${character.name}:`,
              result.voice
            );
          }
        })
        .catch(() => {
          // Silently fail - voice will load on first use
        });
    }
    ...
    try {
      ...
      if (documentId) {
        ...
        try {
          ...
          if (
            ...
            console.log(
              "✅ Found previous history! Loading",
              historyData.conversation_history.length,
              "messages"
            );
      ...
      const result = await getCharacterGreeting(
        documentId,
        character.character_id
      );
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Generate voice in parallel (don't block the UI)
      if (isVoiceActive && fullResponseText && aiVoiceAvailable) {
        speakWithAIVoiceParallel(
          fullResponseText,
          selectedCharacter.character_id
        ).catch((err) => {
          ...
          <div className="book-info">
            ...
            <div className="book-meta">
              ...
              <p className="book-subtitle">
                {book.author || (book.movie_id ? "Movie" : "Book")}
              </p>
            </div>
          </div>

          {selectedCharacter && voiceEnabled && (
            <button
              className={`voice-toggle-button ${isVoiceActive ? "active" : ""}`}
              onClick={toggleVoice}
              title={isVoiceActive ? "Voice Output: ON" : "Voice Output: OFF"}
            >
                ...
                <div className="save-to-library-container">
                  ...
                  {!savedToLibrary && (
                    <p className="save-hint">
                      Save this book to access it anytime from your library
                    </p>
                  )}
                </div>
              )}
            ...
            <div className="delete-confirm-modal">
              <div className="modal-content">
                ...
                <p>
                  ...
                  <strong>"{book.title}"</strong> from your library?
                </p>
            ...
            >
              ...
              >
                {(() => {
                  const characterImages = {
                    ...
                    harkonnen:
                      "/books_images/Dune/Du_Baron Vladimir Harkonnen.png",
                    vladimir:
                      "/books_images/Dune/Du_Baron Vladimir Harkonnen.png",
                    ...
                    // Percy Jackson
                    percy:
                      "/books_images/Percy Jackson & The Olympians/PJC_Percy Jackson.png",
                    annabeth:
                      "/books_images/Percy Jackson & The Olympians/PJC_Annabeth Chase.png",
                    grover:
                      "/books_images/Percy Jackson & The Olympians/PJC_Grover Underwood.png",
                    luke: "/books_images/Percy Jackson & The Olympians/PJC_Luke Castellan.png",
                    chiron:
                      "/books_images/Percy Jackson & The Olympians/PJC_Chiron.png",
                    nico: "/books_images/Percy Jackson & The Olympians/PJC_Nico di Angelo.png",
                    kronos:
                      "/books_images/Percy Jackson & The Olympians/PJC_Kronos.png",

                    // Sherlock Holmes
                    sherlock:
                      "/books_images/Sherlock Holmes/SH_Sherlock Holmes.png",
                    watson:
                      "/books_images/Sherlock Holmes/SH_Dr. John Watson.png",
                    hudson: "/books_images/Sherlock Holmes/SH_Mrs. Hudson.png",
                    moriarty:
                      "/books_images/Sherlock Holmes/SH_Professor Moriarty.png",
                    lestrade:
                      "/books_images/Sherlock Holmes/SH_Inspector Lestrade.png",

                    // Chronicles of Narnia
                    aslan:
                      "/books_images/The Chronicles of Narnia/CN_Aslan.png",
                    lucy: "/books_images/The Chronicles of Narnia/CN_Lucy Pevensie.png",
                    edmund:
                      "/books_images/The Chronicles of Narnia/CN_Edmund Pevensie.png",
                    peter:
                      "/books_images/The Chronicles of Narnia/CN_Peter Pevensie.png",
                    susan:
                      "/books_images/The Chronicles of Narnia/CN_Susan Pevensie.png",
                    caspian:
                      "/books_images/The Chronicles of Narnia/CN_Prince Caspian.png",
                    witch:
                      "/books_images/The Chronicles of Narnia/CN_The White Witch.png",

                    // Diary of a Wimpy Kid
                    greg: "/books_images/Diary of a Wimpy Kid/DWK_Greg Heffley.png",
                    rowley:
                      "/books_images/Diary of a Wimpy Kid/DWK_Rowley Jefferson.png",
                    rodrick:
                      "/books_images/Diary of a Wimpy Kid/DWK_Rodrick Heffley.png",
                    frank:
                      "/books_images/Diary of a Wimpy Kid/DWK_Frank & Susan Heffley.png",
                    manny:
                      "/books_images/Diary of a Wimpy Kid/Manny Heffley.png",
                    ...
                    haymitch:
                      "/books_images/Hunger games/Haymitch Abernathy.png",
                    abernathy:
                      "/books_images/Hunger games/Haymitch Abernathy.png",
                    primrose:
                      "/books_images/Hunger games/Primrose Everdeen.png",
                    ...
                    capitol:
                      "/books_images/Hunger games/Capitol Government (Generic Capitol Enforcer).png",
                    enforcer:
                      "/books_images/Hunger games/Capitol Government (Generic Capitol Enforcer).png",
                    ...
                    oakenshield:
                      "/books_images/The Hobbit/Thorin Oakenshield.png",
                    ...
                    // Frankenstein
                    victor:
                      "/books_images/Frankenstein/Victor Frankenstein.png",
                    frankenstein:
                      "/books_images/Frankenstein/Victor Frankenstein.png",
                    ...
                    sparrow:
                      "/Movie/Pirates of the Caribbean/POC_Captain Jack Sparrow.png",
                    elizabeth:
                      "/Movie/Pirates of the Caribbean/POC_Elizabeth Swann.png",
                    swann:
                      "/Movie/Pirates of the Caribbean/POC_Elizabeth Swann.png",
                    will: "/Movie/Pirates of the Caribbean/POC_William Will Turner.png",
                    turner:
                      "/Movie/Pirates of the Caribbean/POC_William Will Turner.png",
                    william:
                      "/Movie/Pirates of the Caribbean/POC_William Will Turner.png",
                    barbossa:
                      "/Movie/Pirates of the Caribbean/POC_Captain Hector Barbossa.png",
                    hector:
                      "/Movie/Pirates of the Caribbean/POC_Captain Hector Barbossa.png",

                    // Movie: Charlie and the Chocolate Factory
                    charlie:
                      "/Movie/Charlie and the Chocolate Factor/CCF_Charlie Bucket.png",
                    bucket:
                      "/Movie/Charlie and the Chocolate Factor/CCF_Charlie Bucket.png",
                    willy:
                      "/Movie/Charlie and the Chocolate Factor/CCF_Willy Wonka.png",
                    wonka:
                      "/Movie/Charlie and the Chocolate Factor/CCF_Willy Wonka.png",
                    augustus:
                      "/Movie/Charlie and the Chocolate Factor/CCF_Augustus Gloop.png",
                    gloop:
                      "/Movie/Charlie and the Chocolate Factor/CCF_Augustus Gloop.png",
                    veruca:
                      "/Movie/Charlie and the Chocolate Factor/CCF_Veruca Salt.png",
                    salt: "/Movie/Charlie and the Chocolate Factor/CCF_Veruca Salt.png",
                    violet:
                      "/Movie/Charlie and the Chocolate Factor/CCF_Violet Beauregarde.png",
                    beauregarde:
                      "/Movie/Charlie and the Chocolate Factor/CCF_Violet Beauregarde.png",
                    mike: "/Movie/Charlie and the Chocolate Factor/CCF_Mike Teavee.png",
                    teavee:
                      "/Movie/Charlie and the Chocolate Factor/CCF_Mike Teavee.png",
                    grandpa:
                      "/Movie/Charlie and the Chocolate Factor/CCF_Grandpa Joe.png",
              ...
              <p className="character-desc">
                {selectedCharacter.description
                  ? selectedCharacter.description.replace(/☁️|☁/g, "").trim()
                  : ""}
              </p>
            </div>
            <button
